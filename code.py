...
with tab2:
    ...
    fig1 = px.bar(avg_sleep_duration, x='Age', y='Sleep Duration', title='Sleep Duration by Gender', color = 'BMI Category', barmode='group')
    st.plotly_chart(fig1)
...
with tab3:
    ...
    fig1 = px.bar(avg_sleep_duration, x='Age', y='Sleep Duration', title='Sleep Duration by Disorder', color = 'Gender', barmode='group')
    st.plotly_chart(fig1)